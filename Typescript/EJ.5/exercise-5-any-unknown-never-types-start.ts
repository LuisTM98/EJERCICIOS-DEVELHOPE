...
let currency: any = { name: "Indian rupee" };   //Added type any to the currency, so it accepts objects.
...
function countryNameLength(value: unknown) {
    if (typeof value === "string") {            //Changed number to string
...
function throwCountryError(message: string): never {               //Changed variable message to type string.
